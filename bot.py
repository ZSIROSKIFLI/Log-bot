...
def get_user_data(user_id: str):
    if user_id not in activity_data:
        activity_data[user_id] = {"sessions": [], "total_seconds": {}}
...
async def stats(interaction: discord.Interaction, member: discord.Member = None):
    ...
    lines = []
    for game, seconds in sorted_games[:15]:
...
async def on_ready():
    ...
    print(f"{len(activity_data)} felhasznalo adata betoltve")

    # Induláskor végigmegy mindenkin és felveszi az éppen aktív sessionöket
    now = datetime.utcnow()
    count = 0
    for guild in bot.guilds:
        for member in guild.members:
            if member.bot:
                continue
            if not has_figyelt_rang(member):
                continue
            for activity in member.activities:
                if activity.type == discord.ActivityType.playing:
                    ...
                    if uid not in active_sessions:
                        ...
                        count += 1
                        print(f"[STARTUP] {member.display_name} mar jatszik: {activity.name}")
                    break
    print(f"[STARTUP] {count} aktiv session detektálva induláskor")

# ─── Indítás ───────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    TOKEN = os.getenv("DISCORD_TOKEN")
    if not TOKEN:
        print("HIBA: DISCORD_TOKEN kornyezeti valtozo nincs beallitva!")
        exit(1)
    bot.run(TOKEN)
